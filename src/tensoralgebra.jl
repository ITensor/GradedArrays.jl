...
# `check_input(contract, …)` and `default_contract_algorithm` for the graded contraction path
# dispatch on `FusionArray`, so they live in `gradedarrayinterface.jl` (included after it).

...
# `AbstractFusedArray`: it is unavailable on the matrix too, whose adjoint does not carry the same
# sector requirements (see `fusedgradedmatrix.jl`). The `MAK.one!` guard is in `matrixalgebrakit.jl`,
# where `MatrixAlgebraKit` is in scope.

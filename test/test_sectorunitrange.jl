...
using GradedArrays: GradedArrays, GradedUnitRange, SU2, SectorRange, SectorUnitRange, U1,
    dual, flip, gradedrange, isdual, label, labels, sector, sector_multiplicities,
...
@testset "SectorUnitRange" begin
    @testset "U1 construction and accessors" begin
        si = SectorUnitRange(TKS.U1Irrep(1), 3, false)
    ...
    @testset "SU2 construction and accessors" begin
        ...
        si = SectorUnitRange(TKS.SU2Irrep(1 // 2), 4, false)
    ...
    @testset "flip with SU2" begin
        si = SectorUnitRange(TKS.SU2Irrep(1 // 2), 2, false)
        sif = flip(si)
    ...
    @testset "equality" begin
        si1 = SectorUnitRange(TKS.U1Irrep(1), 3, false)
        si2 = SectorUnitRange(TKS.U1Irrep(1), 3, false)
        si3 = SectorUnitRange(TKS.U1Irrep(1), 3, true)
        si4 = SectorUnitRange(TKS.U1Irrep(2), 3, false)
        si5 = SectorUnitRange(TKS.U1Irrep(1), 4, false)
    ...
    @testset "sector_type" begin
        @test sector_type(SectorUnitRange{TKS.U1Irrep}) == SectorRange{TKS.U1Irrep}
        @test sector_type(SectorUnitRange{TKS.SU2Irrep}) == SectorRange{TKS.SU2Irrep}
    end

    @testset "show" begin
        si = SectorUnitRange(TKS.U1Irrep(1), 3, false)
        str = sprint(show, si)
        @test contains(str, "SectorUnitRange")
        @test contains(str, "3")

        sid = SectorUnitRange(TKS.U1Irrep(1), 3, true)

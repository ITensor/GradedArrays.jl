using GradedArrays:
    GradedArrays, Fib, GradedIndices, Ising, O2, SU2, TrivialSector, U1, dual, gradedrange
...
@testset "show GradedIndices" begin
    x = U1(0)
    y = U1(1)
    z = U1(2)
    g1 = gradedrange([x => 2, y => 3, z => 2])
    @test g1 isa GradedIndices

    lx = repr(TKS.U1Irrep(0))
    ly = repr(TKS.U1Irrep(1))
    lz = repr(TKS.U1Irrep(2))
    @test sprint(show, g1) == "GradedIndices([$lx => 2, $ly => 3, $lz => 2])"

    g1d = dual(g1)
    @test sprint(show, g1d) == "GradedIndices([$lx => 2, $ly => 3, $lz => 2])'"
end
